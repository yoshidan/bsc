...
func (p *Parlia) assembleVoteAttestation(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	if targetHeaderParentSnap == nil {
		log.Info(fmt.Sprintf("no vote header=%s, source=%d", header.Number.String(), int64(justifiedBlockNumber)))
		return nil
	}
	log.Info(fmt.Sprintf("has vote header=%s, target=%s, source=%d", header.Number.String(), targetHeader.Number.String(), int64(justifiedBlockNumber)))
